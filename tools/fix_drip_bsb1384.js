/**
 * Faucet drip: BigSoundBank #1384 Drops of water #1 (CC0).
 * Minimal processing so it stays like a real leaky faucet.
 */
...
const TMP = path.join(AUDIO, "_tmp_drip1384");
...
async function main() {
  ...
  await download("https://bigsoundbank.com/UPLOAD/mp3/1384.mp3", raw);
  console.log("raw", probe(raw).toFixed(2) + "s", "max", maxVol(raw));

  // Keep character: light trim + gentle gain only (no heavy EQ/loudnorm)
  const clean = path.join(TMP, "clean.wav");
  run([
    "-i",
    raw,
    "-af",
    "silenceremove=start_periods=1:start_threshold=-50dB:start_silence=0.05,volume=4.5dB",
    "-ac",
    "1",
    "-ar",
    "44100",
  ...
  run([
    ...
    "30",
    "-af",
    "afade=t=in:st=0:d=0.1,afade=t=out:st=29.6:d=0.35",
    "-ac",
    "1",
    "-ar",
    "44100",
  ...
  if (sound) {
    ...
    sound.license = "CC0";
    sound.attribution = "Water drops (BigSoundBank #1384, CC0)";
    sound.mixkit_id = 0;
    sound.mixkit_title = "";
    sound.mixkit_url = "";
